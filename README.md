...
- **Index** — four works in two groups, each on a glass tile that raises and resolves out of a pixel dissolve on
...
Work
- 03 **Experiments** — Field, Explode, Bloom: three ways an image becomes an event. The card is a
  tri-tile, one live miniature per machine; the CTA opens `lab/` · `lab/`
- 04 **Tutor Oriel** — a tutoring platform's registration and course selection, rebuilt after usability
...
main.js             hero field, ambient sprays + click blooms, drawn cursor cell, word pills, card miniatures (the 03 tri-tile), the 04 cover mosaic, counter
